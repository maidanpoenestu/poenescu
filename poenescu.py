...
with SB(uc=True, test=True) as sb:
    ...
    if True:
        ...
        if sb.is_element_visible('#injected-channel-player'):
            oeie = sb.get_new_driver(undetectable=True)
            oeie.uc_open_with_reconnect(url, 5)
            oeie.uc_gui_click_captcha()
            oeie.uc_gui_handle_captcha()
            sb.sleep(10)
            if oeie.is_element_present('button:contains("Accept")'):
                oeie.uc_click('button:contains("Accept")', reconnect_time=4)
